...
{ TGridHeaderSections }
  ...
  public
    ...
    property OwnerSection: TGridHeaderSection read FOwner;  
...
{$ENDIF}
  ...
  public
    ...
    property ThemeXPEnabled: Boolean read FThemeXPEnabled write SetThemeXPEnabled default True;
...
begin
 if Value = FThemeXPEnabled then exit;
...
begin
  ...
  with Params do
  begin
    Style := Style or WS_TABSTOP or BorderStyles[FBorderStyle];
...
begin
  ...
  {$IFDEF EX_D7_UP}
  if FThemeXPEnabled and ThemeServices.ThemesEnabled then begin
    { проверяем мышку над заголовком }
...
begin
  ...
  begin
    ...
    if FThemeXPEnabled and ThemeServices.ThemesEnabled then begin
      ...
      Details := ThemeServices.GetElementDetails(ThemedHeader);
      DC := Canvas.Handle;
      // Header shape
...
begin
 if FHotHeaderSection = Section then exit;
 if Assigned(Section) then begin
  AllowClick := true;
  HeaderClicking(Section, AllowClick);
  if not AllowClick then Section := Nil;
 end;
...
begin
  ...
  {$IFDEF EX_D7_UP}
  if not FThemeXPEnabled or not ThemeServices.ThemesEnabled then
  {$ENDIF}
  { рисуем разделитель }
  with Canvas do
  begin
...
begin
  ...
  begin
    ...
    if Row > 0 then Row := Row div Rows.Height;
    { проверяем ячейку }
