...
uses Windows, Classes, Controls, StdCtrls, Sysutils, Graphics, Buttons,
  Forms, ExtCtrls, Dialogs, Printers, DB, QRPrntr, QR4Const,
...
uses
  QRCtrls, CommDlg, QRPrnSu {$ifdef FRAMELINES}, qrframelines {$endif};
...
{$ifdef FRAMELINES}
  ...
  begin
    ...
    begin
        for i := 0 to parentband.Controlcount-1 do
        begin
            if parentband.Controls[i] is TQRFrameline then
            begin
               tempx := TQRFrameline(parentband.Controls[i]).Left + ( TQRFrameline(parentband.Controls[i]).width div 2);
               tempx := arect.Left + round( 1.0*(arect.Right-arect.Left)* tempx/parentband.Width);
...
begin
  ...
  // standard printer
  // should use a method to get the current printer paper size,
  // that doesn't use getprintersettings() !!
...
{$ifdef PKIDE}
     ...
     begin
       ...
       begin
           if qrprinter.Canvas <> nil then
           begin
              result := round(qrprinter.Canvas.TextWidth(aText)*96.0/qrprinter.aPrinterSettings.PixelsPerX);
              exit;
           end;
       end;
     end;
...
begin
  ...
  begin
    ...
    begin
      aReport := TQuickRep(Reports[I]);
      //if assigned(aReport.BeforePrint) then
      //           aReport.BeforePrint( aReport,reportCancelled);
...
begin
  ...
  FQRPrinter.Title := firstrep.ReportTitle;
