...
uses
  ...
  ComCtrls, Buttons, Printers, Graphics, Forms, DB, QR4Const, Dialogs,
...
const
   cQRName = 'QuickReport 4.07'; { This string should not be resourced }
   cQRVersion = 40;
...
{$ifdef EXTRA}
    ...
    //if DevMode <> nil then FreeMem(DevMode );
...
{$ifdef EvalVersion}
  ...
  begin
    ShowMessage('This evaluation copy of QuickReport only works while Delphi is running. '+
                'Please see http://www.qusoft.com to order a full version');
...
begin
  FShowingPreview := false; // added 24/03/05 for qrphandler
  if Master <> nil then
