...
begin
  ...
  begin
    ...
    R := Bounds(L + TI.X, T + TI.Y, W - TI.X, H);
...
begin
  ...
  if (Grid <> nil) and Grid.IsCellValid(Grid.EditCell) then
  begin
    Self.ReadOnly := Grid.IsCellReadOnly(Grid.EditCell);
    Self.MaxLength := Grid.Columns[Grid.EditCell.Col].MaxLength;
    Self.WantReturns := Grid.Columns[Grid.EditCell.Col].WantReturns;
    Self.WordWrap := Grid.Columns[Grid.EditCell.Col].WordWrap;
    Self.EditMask := Grid.GetEditMask(Grid.EditCell);
...
begin
  ...
  begin
    ...
    begin
      ...
      if gkReturn in CursorKeys then SetCursor(GetCursorCell(CellFocused, goNext), True, True);
    end
    else
  ...
  if (Button = mbLeft) or ((Button = mbRight) and RightClickSelect) then
    ...
    begin
      ...
      begin
        ...
        { проверяем начало редактирования }
        if (ssDouble in Shift) and IsCellEqual(C, CellFocused) and AllowEdit then
        begin
          { показываем строку ввода }
          Editing := True;
...
begin
  ...
  begin
    ...
    FPatternBitmap.Canvas.Pixels[0, 1] := GetGridLineColor(Color);
    FPatternBitmap.Canvas.Pixels[1, 0] := GetGridLineColor(Color);
  end
  else
  begin
    FPatternBitmap.Canvas.Pixels[0, 0] := GetGridLineColor(Color);
    FPatternBitmap.Canvas.Pixels[1, 1] := GetGridLineColor(Color);
  ...
  begin
    ...
    begin
      ...
      begin
        R.Left := R.Right;
        R.Right := R.Right + W;
        if RectVisible(Canvas.Handle, R) then PaintScrollBtnFrameEx(Canvas.Handle, R, False, Flags[DrawBottomLine]);
      end;
    end
  end;
...
begin
  ...
  begin
    { получаем текст заголовка и направление сортировки }
    T := Section.Caption;
    TL := Length(T);
  ...
  begin
    ...
    begin
      { рисуем рамку кнопки }
      PaintScrollBtnFrame(Handle, Rect, IsHeaderPressed(Section));
...
const
  Flags = BF_LEFT or BF_TOP or BF_BOTTOM;
var
  R: TRect;
begin
  { подзаголовки }
  ...
  begin
    Canvas.Brush.Color := Header.Color;
    Canvas.FillRect(R);
    if not Header.Flat then PaintScrollBtnFrameEx(Canvas.Handle, R, False, Flags);
  end;
  { серая полоска снизу }
  if Header.Flat then
  begin
...
procedure TCustomGridView.WndProc(var Message: TMessage);
begin
  { иногда даже при нажатии мышкой таблица не получает фокуса }
  case Message.Msg of
...
begin
  Result := ReadOnly or (not IsCellValid(Cell)) or
    (Cell.Col < Fixed.Count) or Columns[Cell.Col].ReadOnly;
end;
...
const
  CellOffsets: array[Boolean] of TGridCursorOffset = (goPrev, goNext);
var
  Section: TGridHeaderSection;
  SortDir: TGridsortDirection;
  Cell: TGridCell;
begin
  if (not IsCellValid(CellFocused)) or (not IsCellAcceptCursor(CellFocused)) then
  begin
    SortDir := gsNone;
    { получаем сортировку колонки }
    Section := GetHeaderSection(CellFocused.Col, -1);
    if Section <> nil then SortDir := GetSortDirection(Section);
    { ищем доступную ячейку }
    Cell := GetCursorCell(CellFocused, CellOffsets[SortDir = gsAscending]);
    if IsCellEqual(Cell, CellFocused) then Cell := GetCursorCell(CellFocused, CellOffsets[SortDir = gsAscending]);
    { вокруг доступной нет - ищем первую попавшуюся }
    if IsCellEqual(Cell, CellFocused) then Cell := GetCursorCell(CellFocused, goFirst);
    { устанавливаем ее }
    SetCursor(Cell, FCellSelected, True);
  end
  else
    { подправляем выделение ячейки }
    SetCursor(CellFocused, CellSelected, False);
end;
...
var
  ...
  EditText: string;
begin
  if (not ReadOnly) and (Edit <> nil) and (not Edit.ReadOnly) and IsCellValid(EditCell) then
  begin
    EditFocused := Editing;
